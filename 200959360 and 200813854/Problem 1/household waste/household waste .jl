...
# ╔═╡ 849f13b5-e5ab-457c-b2c9-e6906d7f3ba3
...
md"""This notebook contains a prediction model for household waste in dairy product. """

# ╔═╡ cf2ca46a-aeec-4946-9c1a-89ceaf994b13
md"## Data Loading"

# ╔═╡ 1655d8a1-23e6-4503-9caa-43f4617fe7bc
df= DataFrame(CSV.File("./global_food_wastage_dataset.csv", header=1))

# ╔═╡ c534503f-61bf-4b27-9545-d2b865b97887
df1 = select(df, [3,4,5,6,7,8])

# ╔═╡ 3b2d2b15-623c-4b0a-922e-632b96983e33
schema(df1) |> DataFrames.DataFrame

# ╔═╡ 86ba37ac-c322-4b88-966e-02f0e976afc3
Y, X = MLJ.unpack(df1, ==(Symbol("Household Waste (%)")), colname -> true)
...
df2 = filter(row -> row."Food Category" == "Dairy Products", df1)

# ╔═╡ 65f6b642-9561-4f43-95e1-cb14ff10bb39
df3 = select(df2, [2,3,4,5,6])
...
p1 = data(df3) * mapping(:"Total Waste (Tons)", :"Household Waste (%)")

# ╔═╡ 41e24c0d-6866-4f88-9dda-ad3b90490e15
p2 = data(df3) * mapping(:"Avg Waste per Capita (Kg)", :"Household Waste (%)")

# ╔═╡ df52147e-fa23-4c85-870b-ca097f292eed
p3 = data(df3) * mapping(:"Economic Loss (Million \$)", :"Household Waste (%)")

# ╔═╡ 9925cc5a-7b16-4ebd-9504-80d1098ad890
p4 = data(df3) * mapping(:"Population (Million)", :"Household Waste (%)")
...
reg_mod = Reg(solver=Analytical())

# ╔═╡ 3fd40652-3e69-4690-ad7d-21b2cef2363b
hw_pred_machine = machine(reg_mod, x, y)

# ╔═╡ d222b7c5-e240-40cd-8aaf-45df5e898ceb
fit!(hw_pred_machine, rows=train_set, verbosity=0)

# ╔═╡ 3061714e-e4ca-4ed9-bafe-8541a9de8016
evaluate!(hw_pred_machine, resampling=CV(nfolds=10,shuffle=true,rng=42),measures=[rms],repeats=425)

# ╔═╡ edf79464-7d56-4f10-bdd8-525f77c427b9
MLJ.predict(hw_pred_machine, rows=train_set)

# ╔═╡ dcf20ad8-b6d7-4cd6-bd9b-a8256b60618d
hw_pred_params = fitted_params(hw_pred_machine)

# ╔═╡ 37b0ff88-ee4b-4581-9c8d-717b4dba6719
fitted_params(hw_pred_machine)

# ╔═╡ 2c37076a-e9a5-44db-9576-21d5b71ab936
rmse(MLJ.predict(hw_pred_machine, x[train_set,:]), y[train_set])

# ╔═╡ b83a5070-a930-4f27-9b09-c37249b3568b
rmse(MLJ.predict(hw_pred_machine, x[test_set,:]), y[test_set])

# ╔═╡ 4786cb86-9f37-4bf1-8b27-73d9d81892b9
r_lambda = range(reg_mod, :(lambda), lower=1, upper=300, scale=:log10)
...
lin_mod2 = LReg1(solver=Analytical())

# ╔═╡ 8a83b249-2927-4495-a9b1-570b765a064f
hw_lin_mach = machine(lin_mod2, x, y)

# ╔═╡ b3920bc0-13f5-4fd4-a0d6-2d156dbd2bb6
fit!(hw_lin_mach, rows=train_set, verbosity=0)

# ╔═╡ 18972c40-4d30-4f9f-8cad-736efe69a911
evaluate!(hw_lin_mach, resampling=CV(nfolds=10,shuffle=true,rng=42),measures=[rms],repeats=3)

# ╔═╡ b35e9bc3-f716-4d2e-91e3-4a2c67d5e334
hw_lin_params = fitted_params(hw_lin_mach)

# ╔═╡ 2f214bf3-a495-43dd-aad5-fd7524221a21
fitted_params(hw_lin_mach)

# ╔═╡ dbb36a55-8590-4154-83bc-8ebd398722ec
rmse(MLJ.predict(hw_lin_mach, x[train_set,:]), y[train_set])

# ╔═╡ 643683d9-30b1-4d4d-aa5b-9fe2562275c9
rmse(MLJ.predict(hw_lin_mach, x[test_set,:]), y[test_set])

# ╔═╡ 81dc08e0-96f3-4615-bc43-da0cf390c39e
md"### Select Features"

# ╔═╡ 4cac2ae0-d41b-4364-89b7-eec3427c66e8
s_df = select(df3, [1,2,3,5])

# ╔═╡ bc7e38de-95ed-49d5-9f6a-dbd4d200613e
describe(s_df)

# ╔═╡ 1ea616da-6b67-4d33-afd6-53a978e9f8ad
y_s, x_s = MLJ.unpack(s_df, ==(Symbol("Household Waste (%)")), colname -> true)

# ╔═╡ 7bd755ac-8e22-409e-9993-d6ff85835ee8
x_stdz_mach = machine(Standardizer(), x_s)

# ╔═╡ 21b6e8f9-cf85-44de-b3c0-81d8e939d09f
norm_x =MLJ.transform(fit!(x_stdz_mach), x_s)

# ╔═╡ 07248ccb-efdb-4678-86e8-62128d632d62
y_stdz_mach = machine(Standardizer(), y_s)

# ╔═╡ 5e74a236-97f6-4eb3-8572-38f2faaeb5bb
norm_y =MLJ.transform(fit!(y_stdz_mach), y_s)

# ╔═╡ 88b92a82-4033-404e-8da6-4bf79056732a
train_set1, test_set1 = partition(eachindex(norm_y), 0.85;rng=Xoshiro(425))

# ╔═╡ bfb324f2-00ea-46e6-ac5a-d7b29945e42a
md"### RidgeRegressor Model"

# ╔═╡ dc8c0f3e-5821-4556-97f4-702076656588
reg_mod4 = Reg(solver=Analytical())

# ╔═╡ 2be1989c-0034-461b-ac3e-11975856eed0
hw_reg_mach1 = machine(reg_mod4, norm_x, norm_y)

# ╔═╡ 89350d2e-78c1-4bc0-a2ca-5bdb2c030c99
fit!(hw_reg_mach1, rows=test_set)

# ╔═╡ cbb56f6b-2882-4ae5-9805-3907dd46dc91
evaluate!(hw_reg_mach1, resampling=CV(nfolds=10,shuffle=true,rng=42),measures=[rms],repeats=3)

# ╔═╡ 69e16282-57e4-4658-92cd-d56c5b1691de
hw_reg_params1 = fitted_params(hw_reg_mach1)

# ╔═╡ 89aec73b-350d-4671-acf4-807b64506825
fitted_params(hw_reg_mach1)

# ╔═╡ a5eba9d6-82bb-4f6e-8084-04117958e2a1
rmse(MLJ.predict(hw_reg_mach1, norm_x[train_set,:]), norm_y[train_set])

# ╔═╡ 2eb29d6e-6958-48ab-9f6c-c4e113bdf64a
rmse(MLJ.predict(hw_reg_mach1, norm_x[test_set,:]), norm_y[test_set])
...
reg_mod3 = Reg(solver=Analytical())

# ╔═╡ d4f6ee7d-aad0-426e-a9e9-e8f184429a92
hw_pred_machine1 = machine(reg_mod3, norm_x, norm_y)

# ╔═╡ 514d3244-3c79-479b-bfde-78f6a9e832af
fit!(hw_pred_machine1, rows=train_set)

# ╔═╡ b5c4701d-c5d5-407b-ad85-f8912bb30345
evaluate!(hw_pred_machine1, resampling=CV(nfolds=10,shuffle=true,rng=42),measures=[rms],repeats=3)

# ╔═╡ 4c1277c0-f35e-457a-a778-c4508d6ba0ea
hw_pred_params2 = fitted_params(hw_pred_machine1)

# ╔═╡ 41d32f87-f820-4bde-9e55-168ce1ebf514
fitted_params(hw_pred_machine1)

# ╔═╡ 60f77992-b0ac-460c-846b-9e09acd47f27
rmse(MLJ.predict(hw_pred_machine1, norm_x[train_set,:]), norm_y[train_set])

# ╔═╡ de664a52-cca4-401a-b49c-2ddf0e97c784
rmse(MLJ.predict(hw_pred_machine1, norm_x[test_set,:]), norm_y[test_set])
